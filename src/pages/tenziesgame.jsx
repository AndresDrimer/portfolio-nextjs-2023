...
export default function tenziesgame() {
  const createTechDetail = data.projects[1].techDetail.map(it=>{
    return(
    ...
    <div className="w-full">
      <div className="w-screen relative h-[20vh] lg:h-[30vh] ">
        ...
        <div className="absolute top-[70%] max-w-[1240px] w-full left-[50%] right-[50%] translate-x-[-50%] translate-y-[-50%] text-white z-10 p-2">
          <h2 className="py-2">{data.projects[1].title}</h2>
          <h3>{data.projects[1].tech}</h3>
        </div>
      </div>
      <div className="w-screen h-full max-width-[1240px] flex justify-center">
      <Image
        src={data.projects[1].imageInside}
        alt="/"
