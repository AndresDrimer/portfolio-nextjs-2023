...
export default function artistwebsite() {
  const createTechDetail = data.projects[2].techDetail.map(it=>{
    return(
    ...
    <div className="w-full">
      <div className="w-screen relative h-[20vh] lg:h-[30vh] ">
        ...
        <div className="absolute top-[70%] max-w-[1240px] w-full left-[50%] right-[50%] translate-x-[-50%] translate-y-[-50%] text-white z-10 p-2">
          <h2 className="py-2">{data.projects[2].title}</h2>
          <h3>{data.projects[2].tech}</h3>
        </div>
      </div>
      <div className="w-screen h-full max-width-[1240px] flex justify-center">
      <Image
        src={data.projects[2].imageInside}
        alt="/"
      ...
      <div className="max-w-[1240px] mx-auto p-2 grid md:grid-cols-5 gap-8 pt-8">
        <div className="col-span-4">
          ...
          <p className="pr-4">{data.projects[2].description}</p>
          <button className="px-8 py-2 mt-4 mr-8"><a href={data.projects[2].externalUrl} target="_blank">Demo</a></button>
          <button className="px-8 py-2 mt-4 "><a href={data.projects[2].gitHubUrl} target="_blank">Code</a></button>
        </div>
